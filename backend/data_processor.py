...
def load_match_stats(directory_path):
    ...
    for filename in os.listdir(directory_path):
        if not filename.endswith("_players_stats.json"):
            continue
...
def process_data(players_file, stats_dir):
    ...
    # ── Adaugă jucătorii sintetici ──
    synthetic_path = players_file.replace("players (1).json", "players_synthetic.json")
    if os.path.exists(synthetic_path):
        df_synthetic = load_players(synthetic_path)
        df_players = pd.concat([df_players, df_synthetic], ignore_index=True)
        print(f"[OK] {len(df_synthetic)} jucători sintetici adăugați.")

    df_stats = load_match_stats(stats_dir)

    # Group by player_id and sum all numeric columns
    df_agg_stats = df_stats.groupby('player_id').sum().reset_index()

    # Merge cu profilurile
    df_final = pd.merge(df_players, df_agg_stats, on='player_id', how='inner')

    # Filtrează jucătorii cu prea puține minute
    df_final = df_final[df_final['minutes_played'] >= 200].copy()

    # Calculează p90 pentru toate metricile
    ...
    try:
        ...
        df_final = pd.merge(df_final, df_tm[['match_name', 'market_value_in_eur']],
